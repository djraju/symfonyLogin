function addExerciseToSession(sessionNo,matches,boolExerciseProgram){
    var exercise = availableExercises[matches[0]];
...
$(function () {
  ...
  $("#exercise_container div.exercise-item").click(function(e){
    ...
    $(document).on("click", "#exercise_programs div.exercise-item", function(e){
        if(e.shiftKey){
            if( $(this).hasClass('exerciseItemShadow') ) return;
            var currSelected = $(this).attr('id');
            var currSelectedIndex = $(this).index();
            var markExSelected = 0;
            if($("#exercise_programs div.exercise-item.exerciseItemShadow").length >= 1){

                var lastSelectedEx = $("#exercise_programs div.exercise-item.exerciseItemShadow").last().attr('id');
                var lastSelectedExIndex = $("#exercise_programs div.exercise-item.exerciseItemShadow").last().index();
                if(currSelectedIndex<lastSelectedExIndex){
                    var minSelectedIndex = $("#exercise_programs div.exercise-item.exerciseItemShadow").first().index();
                    var minSelectedId = $("#exercise_programs div.exercise-item.exerciseItemShadow").first().attr('id');
                    $("#exercise_programs div.exercise-item").each(function(){
                        if(markExSelected == 1){
                            $(this).addClass('exerciseItemShadow');
                            if(minSelectedId == $(this).attr('id')){
                                markExSelected = 0;
                                $("#exercise_programs div.exercise-item.exerciseItemShadow").each( function() {
                                    if( $(this).index() > minSelectedIndex ) {
                                        $(this).removeClass('exerciseItemShadow');
                                    }
                                });
  ...
  $("#1").droppable({
      ...
      drop: function (event, ui) {
          ...
          if(ui.draggable.parent().attr('id') == "exercise_container"){
                ...
                var exercisesCnt = $("#1 div[id=sessions_programs]").find("div.exercises-choosen").length;
                if(exercisesCnt > 0){
                  $('#sessionLi1 span').show();
                }
          }
      }
  });
  ...
  $( "#1" ).find("#sessions_programs").sortable({
    ...
    start: function(event, ui){
      ...
      $(this).sortable("option","cursorAt",{left: 0, top: 0});
    },
    stop:function (event, ui) {
          ...
          if(ui.draggable.parent().attr('id') == "exercise_container"){
            ...
            $("#exercise_container div.exercise-item.exerciseItemShadow").each(function(){
              if($(this).parent().attr('id') == 'exercise_container'){
                ...
                var matches = exerciseStr.match(numberRegex);
                exerciseIds.push(matches);
                addExerciseToSession("3",matches);
                $(this).removeClass('exerciseItemShadow');
              }
            });
        }
          ...
          if(ui.draggable.parent().attr('id') == "exercise_container"){
            ...
            $("#exercise_container div.exercise-item.exerciseItemShadow").each(function(){
              if($(this).parent().attr('id') == 'exercise_container'){
                ...
                var matches = exerciseStr.match(numberRegex);
                exerciseIds.push(matches);
                addExerciseToSession("4",matches);
                $(this).removeClass('exerciseItemShadow');
              }
            });
        }
          ...
          if(ui.draggable.parent().attr('id') == "exercise_container"){
            ...
            $("#exercise_container div.exercise-item.exerciseItemShadow").each(function(){
              if($(this).parent().attr('id') == 'exercise_container'){
                ...
                var matches = exerciseStr.match(numberRegex);
                exerciseIds.push(matches);
                addExerciseToSession("5",matches);
                $(this).removeClass('exerciseItemShadow');
              }
            });
        }
  ...
  $(document).on('click','.exercise-remove', function(){
    ...
    $(document).on('click','#remove-session', function(){
      $('#loading').show();
      $('#current_exercises .tab-content').hide();
      delSessionNo = $('#session-number').val();
      var sessionsCnt = $("#current_exercises").find("ul.nav-tabs").children().length - 1 ;
      if(sessionsCnt == 1){
        var allInnerDivs = $("#"+delSessionNo).find('#sessions_programs').find('div.exercises-choosen');
        $.each( allInnerDivs, function( key1, innerDivObj ) {
          innerDivObj.remove();
        });
        $('#sessionLi'+delSessionNo+' span').hide();
      }
      else {
        $('#addNewSession').show();
        ...
        $('.nav-tabs a[href="#'+newSessionTab+'"]').parent().tab('show');
        ...
        $("#addNewSession").data("current", existingTabsLen);
      }
      rebuildSessionExercisesStr();
      $('#loading').hide();
      $('#current_exercises .tab-content').show();
    });
  
$(document).on('click','#addNewSession', function(){
      var newSessionTabNo = $(this).data('current') + 1;
      ...
      $(newSessionLi).insertAfter('#sessionLi'+$(this).data('current'));
      $(newSessionDiv).insertAfter('#'+$(this).data('current'));

      if(newSessionTabNo == 5){
        $(this).hide();
      }
      else{
        $(this).data('current',newSessionTabNo);
      }
      ...
      $('.nav-tabs a[href="#'+newSessionTabNo+'"]').parent().tab('show');
...
function rebuildSessionExercisesStr(){
  ...
  workoutUpdates = true;
  $('#editWorkoutForm').addClass('dirty');
    $('#editWorkoutForm').find('button[type=submit]').removeAttr('disabled');
    $('#editWorkoutForm').find('button[type=submit]').removeClass('disable');
}